...
static int nt_func_body(FILE *input, Tree *locals, Tree *globals, Tree *functions,
                        Instruction *instr, unsigned *_var_count);
...
int parse(FILE *input, Instruction *first_instr)
{
    ...
    tree_init(&globals);
    tree_init(&functions);
    ret = nt_program(input, &globals, &functions, &first_instr);
                      ...
                      Instruction **first_instr)
{
    ...
    CHECK_VALUE(nt_main(input, globals, functions, first_instr), ret);
...
static int nt_func(FILE *input, Tree *globals, Tree *functions)
{
    ...
    if (token.type == TOKEN_KEYWORD &&
        ...
        CHECK_VALUE(nt_func_body(input, func->locals, globals, functions,
                                 func->first_instr, &(func->local_count)), ret);
                        ...
                        Instruction *instr, unsigned *_var_count)
{
    ...
    if (token.type == TOKEN_KEYWORD &&
        ...
        if (_var_count != NULL)
            *_var_count = count;
        CHECK_VALUE(nt_comp_cmd(input, locals, globals, functions, &instr), ret);
                       ...
                       Instruction **instr)
{
    ...
    CATCH_VALUE(nt_cmd_list(input, locals, globals, functions, instr), ret);
...
{
    int ret;
    CATCH_VALUE(nt_cmd_sublist(input, locals, globals, functions, instr), ret);
...
{
    int ret;
    CHECK_VALUE(nt_cmd(input, locals, globals, functions, instr), ret);
    CHECK_VALUE(t_symbol_catch(input, SEMICOLON), ret);
    CHECK_VALUE(nt_cmd_sublist(input, locals, globals, functions, instr), ret);
                  ...
                  Instruction **instr)
{
    ...
    if (token.type == TOKEN_KEYWORD) {
        if (token.value.value_keyword == KEYWORD_READLN) {
            ...
            CHECK_VALUE(t_symbol(input, PARENTHESIS_L), ret);
            CHECK_VALUE(nt_arg_list_write(input, locals, globals, functions, instr), ret);
            CHECK_VALUE(t_symbol(input, PARENTHESIS_R), ret);
            return SUCCESS;
        } else if (token.value.value_keyword == KEYWORD_IF) {
            CHECK_VALUE(parse_expr(input, locals, globals, functions, NULL, NULL, NULL),
                        ret);
            ...
            CHECK_VALUE(gen_instr(instr, I_NOP, 0, 0, NULL), ret);
            tmp_instr = *instr;
            CHECK_VALUE(parse_expr(input,locals, globals, functions, NULL, instr, NULL),
                        ret);
            ...
            *instr = tmp_instr2->alt_instruction;
            ...
            CHECK_VALUE(gen_instr(instr, I_NOP, 0, 0, NULL), ret);
            tmp_instr = *instr;
            CHECK_VALUE(nt_cmd_list(input, locals, globals, functions, instr), ret);
        ...
        CHECK_VALUE(parse_expr(input, locals, globals, functions, NULL, instr, NULL), ret);
...
{
    int ret;
    CHECK_VALUE(nt_comp_cmd(input, NULL, globals, functions, first_instr), ret);
